...
[盤面を表示する](./getting-started/displaying-board)では、3つの表示方式を紹介しました。これらの方式によってオプションの指定方法がやや異なります。

## マークアップ方式

マークアップ方式では、`data-オプション名="オプション値"` というような属性を、マークアップの`<script>`タグに付与することで指定します。
