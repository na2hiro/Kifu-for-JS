...
export default class Shogi {
    ...
    public move(fromx: number, fromy: number, tox: number, toy: number, promote: boolean = false): void {
        ...
        if (piece == null) { throw new Error("no piece found at " + fromx + ", " + fromy); }
    ...
    public unmove(
        ...
        if (piece == null) { throw new Error("no piece found at " + tox + ", " + toy); }
    ...
    public drop(tox: number, toy: number, kind: string, color: Color = this.turn): void {
        ...
        if (this.get(tox, toy) != null) { throw new Error("there is a piece at " + tox + ", " + toy); }
    ...
    public undrop(tox: number, toy: number): void {
        const piece = this.get(tox, toy);
        if (piece == null) { throw new Error("there is no piece at " + tox + ", " + toy); }
        this.checkTurn(Piece.oppositeColor(piece.color));
    ...
    public getMovesFrom(x: number, y: number): IMove[] {
        ...
        if (moveDef.just) {
            for (const def of moveDef.just) {
                ...
                if (legal(to.x, to.y, piece.color)) { ret.push({from, to}); }
            }
        }
        if (moveDef.fly) {
    ...
    private checkTurn(color: Color): void {
        if (!this.flagEditMode && color !== this.turn) { throw new Error("cannot move opposite piece"); }
    }
}
