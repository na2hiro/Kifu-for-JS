...
const SettingsIcon = forwardRef<SVGSVGElement, IProps>(({ width, height, kifuStore }, ref) => {
    ...
    }, [(ref as RefObject<SVGSVGElement>).current, openClose.close]);
    ...
    return (
        <>
            ...
            <foreignObject
                ...
                height={height}
                style={{
                    transform: `scale(${openClose.state === OpenCloseState.OPEN ? 1 : 0})`,
                    opacity: openClose.state === OpenCloseState.OPEN ? 1 : 0,
                    transition: "transform 0.16s ease, opacity 0.16s ease",
                }}
            >
                {openClose.state !== OpenCloseState.CLOSED && (
                    <FocusTrap focusTrapOptions={{ clickOutsideDeactivates: true }}>
                        <SettingsModal onClose={openClose.close} kifuStore={kifuStore} />
                    </FocusTrap>
                )}
            </foreignObject>
        </>
...
function useOpenClose(delay = 200) {
    const [state, setState] = useState(OpenCloseState.CLOSED);

    const open = useCallback(() => setState(OpenCloseState.OPEN), []);
    const close = useCallback(() => {
        if (state === OpenCloseState.CLOSED) {
            return;
        }
