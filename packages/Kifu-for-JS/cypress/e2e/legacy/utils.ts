...
export const selectCell = (x: number, y: number) => cy.get(`table tr:nth-child(${y + 1}) td:nth-child(${10 - x})`);
...
export const lastCellShouldBe = (x: number, y: number) =>
    selectCell(x, y).should("have.class", "kifuforjs-cell--lastto");
export const lastCellShouldNotBe = (x: number, y: number) =>
    selectCell(x, y).should("not.have.class", "kifuforjs-cell--lastto");
